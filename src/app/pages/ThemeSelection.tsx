...
function inferTemplateKind(...values: unknown[]): TemplateType {
  ...
  if (text.some((value) => value.includes("fauget hotel") || value.includes("golden hotel") || value.includes("hotel golden")))
    return "template5";
  if (text.some((value) => value.includes("restaurant food menu") || value.includes("food menu") || value.includes("burgundy")))
    return "template6";
  if (text.some((value) => value.includes("burger") || value.includes("fauget")))
    return "template4";
  if (text.some((value) => value.includes("dark")))
    return "template2";
...
function getDefaultsForKind(kind: TemplateType) {
  switch (kind) {
    case "template2": return { defaultConfig: DEFAULT_TEMPLATE2_CONFIG, component: Template2 };
    case "template3": return { defaultConfig: DEFAULT_TEMPLATE3_CONFIG, component: Template3 };
...
function normalizeConfigForKind(kind: TemplateType, config: TemplateConfig, themeName?: string): TemplateConfig {
...
function normalizeSavedConfigs(configs: Record<string, TemplateConfig>): Record<string, TemplateConfig> {
...
export function ThemeSelection() {
  ...
  // planId 2 = Pro, 3 = Premium; 1 = Starter — cannot edit templates
  ...
  useEffect(() => {
    ...
    if (storedActiveThemeId) {
      setActiveThemeId(storedActiveThemeId); // ← and this
      setSelectedTheme(storedActiveThemeId); // ← keeps selection consistent too
    }
    ...
    if (parsed && !isExpired) {
      ...
      if (isExpired) localStorage.removeItem("subscription");
    }
  ...
  useEffect(() => {
    const fetchAvailableThemes = async () => {
      try {
        ...
        const mapped: Theme[] = apiThemes.map((t) => {
          const inferredKind = inferTemplateKind(t.templateType, t.config?.id, t.config?.templateType, t.id, t.name);
        ...
        setThemes(mapped);
        const fetchedActiveId = payload.activeThemeId ?? null;
        setActiveThemeId(fetchedActiveId);
        if (fetchedActiveId) {
          localStorage.setItem("activeThemeId", fetchedActiveId);
        } else {
  ...
  const handleEditTemplate = (themeId: string) => {
    ...
    // Starter plan (planId 1) cannot edit — show upgrade popup
    if (!canEditTemplates) {
  ...
  const getConfigForTheme = (themeId: string): TemplateConfig | undefined => {
    ...
    const config = saved ?? theme.defaultConfig;
    if (!config) return undefined;
    const kind = inferTemplateKind(
      config.id, config.templateType,
      theme.defaultConfig?.id, theme.defaultConfig?.templateType,
      theme.id, theme.name
    );
    return normalizeConfigForKind(kind, config, theme.name);
  };
  ...
  const handleSave = async (config: TemplateConfig) => {
    ...
    const theme = themes.find((t) => t.id === themeId);
    const kind = theme
      ? inferTemplateKind(config.id, config.templateType, theme.defaultConfig?.id, theme.defaultConfig?.templateType, theme.id, theme.name)
    ...
    setSavedConfigs((prev) => {
      const updated = { ...prev, [themeId]: normalizedConfig };
  ...
  const handleApplyTheme = async () => {
    ...
    try {
      const token = getToken();
      if (!token) { setApplyError("Session expired. Please log in again."); return; }

      const appliedConfig = getConfigForTheme(selectedTheme);
      if (!appliedConfig) { setApplyError("No configuration found for this theme."); return; }

      const res = await axios.post(
        `${API}/hotel-admin/theme/apply-theme`,
        { menuThemeId: selectedTheme, appliedConfig },
        { headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` } }
      );

      const data = res.data;
      if (!data.success) { setApplyError(data.message || "Failed to apply theme."); return; }
      ...
      if (nextConfig) localStorage.setItem("activeThemeConfig", JSON.stringify(nextConfig));

      const theme = themes.find((t) => t.id === nextActiveId);
      const inferredKind = theme
        ? inferTemplateKind((nextConfig as TemplateConfig)?.id, (nextConfig as TemplateConfig)?.templateType, theme.defaultConfig?.id, theme.defaultConfig?.templateType, theme.id, theme.name)
      ...
      setApplyError(err.response?.data?.message || "Network error. Please try again.");
    } finally {
      setIsApplying(false);
    }
  };
  ...
  // Only checks component availability — NOT plan tier (button always shown, disabled for starter)
    ...
    <div className="space-y-8">
            ...
            return (
              ...
              >
                ...
                <CardContent className="p-5">
                  ...
                  {/* Always show Edit Template when template is selected — disabled for starter */}
      ...
      <div className="flex justify-end gap-4">
        <Button
          ...
          onClick={() => { if (!requireSubscription()) return; }}
        >
          Preview Menu
        </Button>
        <Button
          ...
          <div className="bg-gray-100 rounded-lg p-4 sm:p-8 flex items-center justify-center overflow-auto">
            {TemplateComponent ? (
              <div className="w-full">
                <TemplateComponent config={getConfigForTheme(selectedTheme)} />
              </div>
            ) : (
      ...
      {showEditor && editorConfig && (
        <TemplateEditor
          ...
          renderPreview={(liveConfig: TemplateConfig) => {
            const editingTheme = themes.find((t) => t.id === (editingThemeId ?? selectedTheme));
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-md">
          <div className="bg-white rounded-3xl w-[92%] max-w-md shadow-2xl overflow-hidden border border-gray-100">
            <div className="relative bg-gradient-to-r from-[#1E88E5] via-[#1976D2] to-[#1565C0] p-7 text-white text-center">
              ...
              <p className="text-sm opacity-90 mt-2">Unlock powerful restaurant management tools</p>
            </div>
            <div className="p-6 space-y-5 text-center">
              ...
              <div className="space-y-3 pt-2">
                <Button
                  ...
                  onClick={() => { setShowPopup(false); window.location.href = "/subscription"; }}
                >
                  View Plans & Upgrade
                </Button>
                <Button
      ...
      {/* Pro/Premium required popup (starter plan clicking Edit Template) */}
      {showUpgradePopup && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-md">
          <div className="bg-white rounded-3xl w-[92%] max-w-md shadow-2xl overflow-hidden border border-gray-100">
            <div className="relative bg-gradient-to-r from-[#7C3AED] via-[#6D28D9] to-[#5B21B6] p-7 text-white text-center">
              <div className="flex justify-center mb-3">
                <div className="bg-white/20 rounded-full p-3">
                  <Lock className="w-6 h-6 text-white" />
                </div>
              </div>
              <h2 className="text-2xl font-bold tracking-tight">Pro or Premium Required</h2>
              <p className="text-sm opacity-90 mt-2">Template editing is a premium feature</p>
            </div>
            <div className="p-6 space-y-5 text-center">
              <div className="bg-purple-50 border border-purple-100 rounded-xl p-4">
                <p className="text-sm text-gray-700">
                  Your <span className="font-semibold text-purple-600">Starter</span> plan doesn't
                  include template editing. Upgrade to{" "}
                  <span className="font-semibold text-purple-600">Pro or Premium</span> to
                  customize your menu layout.
                </p>
              </div>
              <div className="text-left bg-gray-50 p-5 rounded-xl space-y-3 text-sm">
                <p className="font-semibold text-gray-800 mb-2">🎨 With template editing you get:</p>
                <div className="space-y-2 text-gray-600">
                  <p>✏️ Full layout customization</p>
                  <p>🎨 Colors, fonts & spacing control</p>
                  <p>📐 Header & footer configuration</p>
                  <p>💾 Saved configurations per template</p>
                </div>
              </div>
              <div className="space-y-3 pt-2">
                <Button
                  className="w-full bg-gradient-to-r from-[#7C3AED] to-[#6D28D9] hover:opacity-95 text-white font-semibold py-3 rounded-xl shadow-md transition-all"
                  onClick={() => { setShowUpgradePopup(false); window.location.href = "/subscription"; }}
                >
